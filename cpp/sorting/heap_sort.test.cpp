#include <cassert>
#include <iostream>

#include "heap_sort.hpp"
#include "perf_measurer.hpp"

using namespace std;

int main(int argc, char const *argv[]) {
  int arr[] = {76, 34, 67, 32, 78, 45};
  const int length = sizeof(arr) / sizeof(*arr);
  HeapSort<int> *heapSort = new HeapSort<int>();

  cout << "Before: " << heapSort->toString(arr, length) << endl;
  heapSort->sort(arr, length);
  cout << "After: " << heapSort->toString(arr, length) << endl;

  int expected[] = {32, 34, 45, 67, 76, 78};
  for (int i = 0; i < length; i++) {
    assert(arr[i] == expected[i]);
  }

  PerfMeasurer<int> *perfMeasurer =
      new PerfMeasurer<int>((const Sort<int> &)HeapSort<int>());

  cout << perfMeasurer->reportPerf() << endl;

  cout << "Test passed" << endl;

  return 0;
}
